...
impl<P: BasicProblem> Ipopt<P> {
    ...
    /// Open an IPOPT journal output file.
    ///
    /// IPOPT will write its native solver transcript to this file at the requested print level.
    pub fn open_output_file(&mut self, file_name: &str, print_level: i32) -> Option<&mut Self> {
        let file_name_cstr = CString::new(file_name).ok()?;
        let opened = unsafe {
            ffi::cnlp_open_output_file(
                self.nlp_internal,
                file_name_cstr.as_ptr(),
                print_level as Int,
            )
        };
        if opened != 0 { Some(self) } else { None }
    }
